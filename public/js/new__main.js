...
var kunsthalle = {
	...
	initCalendar: function () {
		...
		$('#calendar')
			...
			.on('click', '#filter ul a', function (e) {
				...
				// set new filter name
				$('#filter .filter-name').text($(this).text());
			...
			.on('change', '[name="artclubmember"]', function () {
				var memberInput = $(this);
				// get the number input wich depends on the checkbox
				var numberInput = memberInput.parents('form').find('[name="artclubnumber"]');
	...
	initAnchor: function() {
		...
		anchors.each(function() {
			var anchor = $(this);
			var anchorText = anchor.data('anchortext') ? anchor.data('anchortext') : anchor.text();
